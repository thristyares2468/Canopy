        ...
        {section === 'advanced' && <div className="settings-groups">
          ...
          <details className="internal-pages">
            ...
            <div className="internal-content">
              ...
              <label>Local GitHub checkout<input value={draftPath} onChange={event => setDraftPath(event.target.value)} onBlur={saveGameSettings} /></label>
              ...
              <div className="game-actions">
                <button type="button" className="primary-button" onClick={async () => { await saveGameSettings(); onLaunchGame(); }}><Gamepad2 />Open game files</button>
              </div>
              ...
              <p className="internal-note">Canopy serves the checkout locally and connects multiplayer to the online server. Repository files are never copied or modified.</p>
            </div>
          </details>
        </div>}
      </div>
...
export default function App() {
  ...
  const launchGame = async () => {
    if (!api) return;
    setGameStatus({ state: 'starting', message: 'Opening local game files…' });
